(function () { 
	 ...
	 */
	function startStreaming(options, errorCallback, onStreaming, okCallback) {
	...
	function startVideoStreaming(callback, options) {
		...
		if(navigator.getMedia) {
			...
			if(timeoutLength > 0) {
				noGUMSupportTimeout = setTimeout(onStreamingTimeout, timeoutLength);
			}

			startStreaming( options, function() {

					// Already failed setting up, so clear the timeout
